...
class StaticContentModelExport extends JModel
{
	...
	private function _writePages()
	{
		...
		if (!JFolder::exists($base_directory)) {
			if (!JFolder::create($base_directory)) die(JText::sprintf('COM_STATICCONTENT_MSG_FAILURE_CREATE_BASE_DIRECTORY',$base_directory));
		}
		
		//write menus pages
		foreach ($this->_links['menu'] as $menuPage) {
			if (!$this->_writePage($menuPage)) die(JText::sprintf('COM_STATICCONTENT_MSG_FAILURE_CREATE_FILE',$menuPage->file));
		}
		
		//write internal pages
		foreach ($this->_links['pages'] as $internalPage) {
			if (!$this->_writePage($internalPage)) die(JText::sprintf('COM_STATICCONTENT_MSG_FAILURE_CREATE_FILE',$internalPage->file));
		}
